...
@router.get("/")
def get_projects(Authorize: get_auth_wrapper = Depends(get_auth_wrapper)):
    auth_check(Authorize)
    return get_compose_projects()


@router.get("/{project_name}")
def get_project(project_name, Authorize: get_auth_wrapper = Depends(get_auth_wrapper)):
    auth_check(Authorize)
    return get_compose(project_name)


@router.get("/{project_name}/actions/{action}")
def get_compose_action(project_name, action, Authorize: get_auth_wrapper = Depends(get_auth_wrapper)):
    auth_check(Authorize)
    if action == "delete":
...
def get_compose_app_action(project_name, action, app, Authorize: get_auth_wrapper = Depends(get_auth_wrapper)):
    auth_check(Authorize)
    return compose_app_action(project_name, action, app)


@router.get("/{project_name}/support")
def get_support_bundle(project_name, Authorize: get_auth_wrapper = Depends(get_auth_wrapper)):
    auth_check(Authorize)
